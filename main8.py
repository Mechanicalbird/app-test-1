...
def size_messurment_func(counter,calculation_frames,frame,name,input_value,width,height,fps,User_hight,model,model_pose,data_arr_names,width_mes_1,width_mes_2,width_mes_3,width_mes_4,inseem_mes_5,inseem_mes_6,arm_mes_7,arm_mes_8,length_man_9,cm_value_factor_10,User_hight_array_11):
 ...
 ########################## Calculation is started #############################
    ...
    if st.button('run me!'):
        ...
        while True:
            ...
            ret, frame = cap.read()
            print("FRAME_START___XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
            ...
            if counter % calculation_frames == 0 and time.time() - start_time <= 10:
                zero_image = np.zeros((height, width), dtype=np.uint8)
                frame_rgb_before = cv2.cvtColor(zero_image, cv2.COLOR_BGR2RGB)
                #rgb_before = Image.fromarray(frame_rgb_before)
                
                font = cv2.FONT_HERSHEY_SIMPLEX
                input_text_instruction = "--- FRONT ---"
                cv2.putText(frame_rgb_before,input_text_instruction,(int(width/4), int(height/2)), font,1,(255,255,255),2)
                
                video_feed.image(frame_rgb_before, channels="RGB")
                
            
            if counter % calculation_frames == 0 and time.time() - start_time <= 20 and time.time() - start_time >= 10:
                frame_out = size_messurment_func(counter,calculation_frames,frame,name,input_value,width,height,fps,User_hight,model,model_pose,data_arr_names,width_mes_1,width_mes_2,width_mes_3,width_mes_4,inseem_mes_5,inseem_mes_6,arm_mes_7,arm_mes_8,length_man_9,cm_value_factor_10,User_hight_array_11)
                frame_rgb = cv2.cvtColor(frame_out, cv2.COLOR_BGR2RGB)
                img_pil = Image.fromarray(frame_rgb)
                video_feed.image(img_pil, channels="RGB")
            
            
            if counter % calculation_frames == 0 and time.time() - start_time <= 30 and time.time() - start_time >= 20:
                zero_image = np.zeros((height, width), dtype=np.uint8)
                frame_rgb_after = cv2.cvtColor(zero_image, cv2.COLOR_BGR2RGB)
                ...
                video_feed.image(frame_rgb_after, channels="RGB")
                
            
            if counter % calculation_frames == 0 and time.time() - start_time <= 60 and time.time() - start_time >= 30:
                frame_out = size_messurment_func(counter,calculation_frames,frame,name,input_value,width,height,fps,User_hight,model,model_pose,data_arr_names,width_mes_1,width_mes_2,width_mes_3,width_mes_4,inseem_mes_5,inseem_mes_6,arm_mes_7,arm_mes_8,length_man_9,cm_value_factor_10,User_hight_array_11)
                frame_rgb = cv2.cvtColor(frame_out, cv2.COLOR_BGR2RGB)
                img_pil = Image.fromarray(frame_rgb)
                video_feed.image(img_pil, channels="RGB")
            ...
            if counter % calculation_frames == 0 and time.time() - start_time >= 40:
                break
        ...
        st.title("REUSLTS out")
        st.write('Your name is ', name)
        st.write("You entered:", 11111111111+input_value)   
        st.title("REUSLTS end")    
